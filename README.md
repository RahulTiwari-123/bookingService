...
- bookingservApi - This module contains the interface.
    - `v1/schema/bookingService.json` defines the bookingService resource.
    ...
    - `BookingResource.java` is the interface for CRUD operations on `Booking` resource.
        - POST `/v1/bfs/bookingService` endpoint defined to create the resource.
...
- Use postman or curl to access `http://localhost:8080/v1/bfs/bookingService` POST or GET endpoint.

## Assignment
We would like you to enhance the existing project and see you complete the following requirements:

- `bookingService.json` has only `name`, and `id` elements. Please add `date of birth`, `checkin_datetime`, `checkout_datetime`, `totalprice`, `deposit` and `address` elements to the `Booking` resource. Address will have `line1`, `line2`, `city`, `state` and `zip_code` elements. `line2` is an optional element.
